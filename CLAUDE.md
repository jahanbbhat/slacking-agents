...
All feature work follows a two-phase flow:
...
For most features, invoke `slack-orchestrator` directly with a description of what you want to build. It will invoke `slack-architect` to produce a plan, then sequence the implementation agents automatically.

```
User → slack-orchestrator → slack-architect (plan) → implementation agents → code
```
...
2. slash-command-handler builds the /standup endpoint
3. block-kit-builder builds the modal UI
4. slack-event-router wires any needed event listeners
5. slack-message-formatter handles the post to #standups
6. slack-payload-mocker generates test fixtures
```

## Local reference docs

All agents read from `docs/slack/` when they need Slack API specifics. Do not delete or rename these files.
